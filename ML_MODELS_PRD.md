...
type FeatureVector = {
  event_id: string;
  hour_sin: number; hour_cos: number;
  dow_sin: number; dow_cos: number;
  is_peak_hour: boolean; is_weekend: boolean;
...
type ImpactScore = {
  event_id: string;
  p_closure: number;
  ict_p20_h: number; ict_p50_h: number; ict_p80_h: number;
  rci: number;
...
type PropagationMap = {
  event_id: string;
  nodes: Array<{ node_id: string; risk: number; hop: number; parent_edge?: string }>;
  cascade_risk: number;
...
type ReplayBufferManifest = {
  job_id: string;
  recent_pct: number; anchor_pct: number;
  strata_coverage: Record<string, number>;
  bias_weight_version: string;
  promotion_decision: 'PASS' | 'BLOCK';
  metrics: { accuracy: number; pr_auc: number; f1_macro: number; anchor_accuracy: number };
};
```
